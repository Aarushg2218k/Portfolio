    ...
    <div className='w-full max-w-7xl'>
      <div className='md:flex items-center justify-between py-4 md:px-10 px-7'>
        ...
        <div onClick={()=> setopen(!open)} className='w-7 h-7 right-8 top-6 cursor-pointer md:hidden text-white absolute'>
          ...
          {/* Nav Link item */}
          <ul className={`md:flex md:item-center ms-4 md:pb-0 absolute md:static md:z-auto z-10 left-0 w-full md:w-auto mt-4 md:mt-0 md:pl-0 md:pl-9 bg-slate-400 sm:bg-slate-400 md:bg-transparent  sm:bg-transparent transition-all duration-500 ease-in ${open ? "top-12" : "top-[-490px]"}`}>
              {
                Links.map((link)=>{
                  return(
